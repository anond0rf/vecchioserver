...
func NewAPIHandler(userAgent string, verbose bool) *APIHandler {
	...
	return &APIHandler{
		client: client.NewVecchioClientWithConfig(conf),
	}
}

// NewThread Create a new thread on a specific board
func (h *APIHandler) NewThread(ctx echo.Context) error {
	ctx.Logger().Info("New thread request received")

	var body Thread
	if err := ctx.Bind(&body); err != nil {
		ctx.Logger().Errorf("Invalid request body: %v\n", err)
		resp := ErrorResponse{
	...
	ctx.Logger().Infof("Thread to be posted: %+v\n", thread)

	id, err := h.client.NewThread(thread)
	if err != nil {
		ctx.Logger().Errorf("Failed to create thread: %v\n", err)
		resp := ErrorResponse{
	...
	ctx.Logger().Infof("Thread #%d posted successfully\n", id)
...
func (h *APIHandler) PostReply(ctx echo.Context) error {
	ctx.Logger().Info("Post reply request received")
	...
	ctx.Logger().Infof("Reply to be posted: %+v\n", reply)

	id, err := h.client.PostReply(reply)
	if err != nil {
		ctx.Logger().Errorf("Failed to post reply: %v\n", err)
		resp := ErrorResponse{
	...
	ctx.Logger().Infof("Reply #%d posted successfully\n", id)
